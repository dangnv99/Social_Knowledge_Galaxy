...
import { Search as SearchIcon, Filter, Star, Clock, Eye, Heart } from 'lucide-react';
import { useApp } from '../../contexts/AppContext';

export default function Search() {
  const { documents, searchFilters, setSearchFilters, setSelectedDocument } = useApp();

  const filteredDocuments = useMemo(() => {
    if (!searchFilters?.query || !searchFilters.query.trim()) return documents;
  ...
  return (
    <div className="p-6 space-y-8 bg-gray-50 min-h-screen">
      ...
      <div className="text-center bg-white rounded-xl p-8 shadow-sm border border-gray-200">
        ...
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Khám phá Tri thức</h1>
        <p className="text-gray-600">Tìm kiếm và khám phá kho tri thức của tổ chức</p>
      </div>
        ...
        <div className="bg-white rounded-xl p-8 border border-gray-200 shadow-sm mb-8">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-gray-600 text-sm">
              {filteredDocuments.length} tài liệu tìm thấy
            </div>
          </div>
        </div>
        ...
        {searchFilters?.query ? (
          filteredDocuments.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-xl border border-gray-200 shadow-sm">
              ...
              <p className="text-gray-600 mb-6">Không có tài liệu nào phù hợp với từ khóa tìm kiếm</p>
              <button 
              ...
              {filteredDocuments.map((doc) => (
                ...
                >
                  <div className="flex items-start space-x-4">
                    <div className={`w-14 h-14 rounded-full bg-gradient-to-r ${getVisibilityColor(doc.visibility)} flex items-center justify-center text-xl group-hover:scale-110 transition-transform`}>
                    ...
                    <div className="flex-1">
                      <div className="flex items-start justify-between mb-3">
                        <div>
                          ...
                          <p className="text-gray-600 text-sm mb-2">bởi {doc.author}</p>
                        </div>
          ...
          <div className="text-center py-16 bg-white rounded-xl shadow-sm border border-gray-200">
            <div className="text-6xl mb-4">🌌</div>
            <h3 className="text-xl font-semibold text-gray-900 mb-2">Sẵn sàng khám phá?</h3>
            <p className="text-gray-600 mb-6">Nhập từ khóa tìm kiếm để khám phá kho tri thức</p>
